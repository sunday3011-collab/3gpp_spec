...
|  |  |
| ![](images/38.314_L2_Measurements_j00/fig-0001.emf) | ![](images/38.314_L2_Measurements_j00/fig-0002.emf) |
|  |  |
...
4.2.1.3a Number of active UEs in RRC_INACTIVE with ongoing SDT procedure 17
4.2.1.3a.1 General 17
4.2.1.3a.2 Mean number of Active UEs in RRC_INACTIVE with ongoing SDT procedure in the DL per DRB per cell 17
4.2.1.3a.3 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure in the DL per DRB per cell 18
4.2.1.3a.4 Mean number of Active UEs in RRC_INACTIVE with ongoing SDT procedure in the UL per DRB per cell 18
4.2.1.3a.5 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure in the UL per DRB per cell 19
4.2.1.3a.6 Mean number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per cell 20
4.2.1.3a.7 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per cell 21
4.2.1.3a.8 Mean number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per DRB per cell 21
4.2.1.3a.9 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per DRB per cell 22
4.2.1.4 Number of stored inactive UE contexts 23
4.2.1.4.1 General 23
4.2.1.4.2 Mean number of stored inactive UE contexts 23
4.2.1.4.3 Max number of stored inactive UE contexts 24
4.2.1.5 Packet Loss Rate 24
4.2.1.5.1 Packet Uu Loss Rate in the DL per DRB per UE 24
4.2.1.5.2 Packet Uu Loss Rate with delay threshold in the DL per DRB per UE 25
4.2.1.6 Other measurements defined in TS 28.552 [2] 26
4.2.1.7 PRB Usage for MIMO 26
4.2.1.7.1 PDSCH PRB Usage for MIMO in the DL per cell 26
4.2.1.7.2 PUSCH PRB Usage for MIMO in the UL per cell 27
4.2.1.7.3 PDSCH PRB Usage based on statistical MIMO layer in the DL per cell 28
4.2.1.7.4 PUSCH PRB Usage based on statistical MIMO layer in the UL per cell 29
4.2.1.7.5 Enhanced PDSCH PRB Usage for MIMO in the DL per cell 30
4.2.1.7.6 Enhanced PUSCH PRB Usage for MIMO in the UL per cell 31
4.2.1.8 Number of PDCP Packets in Split-DRB Scenario 32
4.2.1.8.1 Number of PDCP Non-duplicated Packets per cell group per Split-DRB per UE 32
4.2.1.8.2 Number of PDCP duplicated Packets per Split-DRB per UE 32
4.2.1.9 Total RAN Delay in Split-DRB Scenario 33
4.2.1.9.1 DL Delay Calculation per Split-DRB per UE when PDCP duplication is enabled for entire measurement period 33
4.2.1.9.2 UL Delay Calculation per Split-DRB per UE when PDCP duplication is enabled for entire measurement period 33
4.2.1.9.3 DL Delay Calculation per Split-DRB per UE when PDCP duplication is not enabled for entire measurement period 34
4.2.1.9.4 UL Delay Calculation per Split-DRB per UE when PDCP duplication is not enabled for entire measurement period 35
4.2.1.10 PDCCH CCE Usage 36
4.2.1.10.1 PDCCH CCE Usage per cell 36
4.2.1.10.2 PDCCH CCE Usage based on statistical multi-UE spatial multiplexing per cell 36
4.3 NR measurements performed by the UE 38
4.3.1 Packet delay 38
4.3.1.1 UL PDCP Packet Average Delay per DRB per UE 38
4.3.1.2 UL PDCP Excess Packet Delay per DRB 39
4.3.1.2.1 Measurement report mapping for PDCP Excess Packet Delay 39
...
| Definition | Maximum number of Active UEs in the DL per DRB per cell. The DRBs are mapped with the same 5QI for NR SA or mapped with the same QCI for EN-DC. This measurement refers to UEs for which there is data available for transmission for the DL for DRBs. Detailed Definition: $M(T,drbid,p)=\underset{T}{max}(\left( N\left( i,drbid \right) \right))$![](images/38.314_L2_Measurements_j00/fig-0003.png), where explanations can be found in the table 4.2.1.3.3-2 below. |
| --- | --- |
...
| Definition | Mean number of Active UEs in the UL per DRB per cell. The DRBs are mapped with the same 5QI for NR SA or mapped with the same QCI for EN-DC. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs. Detailed Definition: $M(T,drbid,p)=\frac{\left⌊ \frac{\sum_{\forall i} N(i,drbid)}{I(T,p)}*10 \right⌋}{10},$![](images/38.314_L2_Measurements_j00/fig-0004.png)where explanations can be found in the table 4.2.1.3.4-2 below. |
| --- | --- |
...
| Definition | Maximum number of Active UEs in the UL per DRB per cell. The DRBs are mapped with the same 5QI for NR SA or mapped with the same QCI for EN-DC. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs. Detailed Definition: $M\left( T,drbid,p \right)=\underset{T}{max}(\left( N\left( i,drbid \right) \right))$, where explanations can be found in the table 4.2.1.3.5-2 below. |
| --- | --- |
...
| Definition | Mean number of Active UEs per cell. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,p)=\frac{\left⌊ \frac{\sum_{\forall i} N(i)}{I(T,p)}*10 \right⌋}{10},$![](images/38.314_L2_Measurements_j00/fig-0005.png)where explanations can be found in the table 4.2.1.3.6-2 below. |
| --- | --- |
...
| Definition | Maximum number of Active UEs per cell. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,p)=\underset{T}{max}(\left( N\left( i \right) \right))$![](images/38.314_L2_Measurements_j00/fig-0006.png), where explanations can be found in the table 4.2.1.3.7-2 below. |
| --- | --- |
...
| Definition | Mean number of Active UEs per DRB per cell. The DRBs are mapped with the same 5QI for NR SA or mapped with the same QCI for EN-DC. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,drbid,p)=\frac{\left⌊ \frac{\sum_{\forall i} N(i,drbid)}{I(T,p)}*10 \right⌋}{10}$, ![](images/38.314_L2_Measurements_j00/fig-0007.png)where explanations can be found in the table 4.2.1.3.8-2 below. |
| --- | --- |
...
| Definition | Maximum number of Active UEs per DRB per cell. The DRBs are mapped with the same 5QI for NR SA or mapped with the same QCI for EN-DC. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,drbid,p)=\underset{T}{max}(\left( N\left( i,drbid \right) \right))$![](images/38.314_L2_Measurements_j00/fig-0008.png), where explanations can be found in the table 4.2.1.3.9-1 below. |
| --- | --- |
...
#### 4.2.1.3a Number of active UEs in RRC_INACTIVE with ongoing SDT procedure
##### 4.2.1.3a.1 General
...
| $$I(T,p)$$ | Total number of sampling occasions during time period $T$. |
| $$T$$ | Time Period during which the measurement is performed, Unit: second. |
| $$drbid$$ | The DRBs mapped with the same 5QI for NR SA. |


##### 4.2.1.3a.3 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure in the DL per DRB per cell
Protocol Layer: MAC, RLC
Table 4.2.1.3a.3-1: Definition for Max number of Active UEs in the DL per DRB per cell
| Definition | Maximum number of Active UEs in the DL per DRB per cell. The DRBs are mapped with the same 5QI for NR SA. This measurement refers to UEs for which there is data available for transmission for the DL for DRBs. Detailed Definition: $M(T,drbid,p)=\underset{T}{max}(\left( N\left( i,drbid \right) \right))$![](images/38.314_L2_Measurements_j00/fig-0009.png), where explanations can be found in the table 4.2.1.3a.3-2 below. |
| --- | --- |
...
##### 4.2.1.3a.4 Mean number of Active UEs in RRC_INACTIVE with ongoing SDT procedure in the UL per DRB per cell
Protocol Layer: MAC
Table 4.2.1.3a.4-1: Definition for Mean number of Active UEs in the UL per DRB per cell
| Definition | Mean number of Active UEs in the UL per DRB per cell. The DRBs are mapped with the same 5QI for NR SA. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs. Detailed Definition: $M(T,drbid,p)=\frac{\left⌊ \frac{\sum_{\forall i} N(i,drbid)}{I(T,p)}*10 \right⌋}{10},$![](images/38.314_L2_Measurements_j00/fig-0010.png)where explanations can be found in the table 4.2.1.3a.4-2 below. |
| --- | --- |
...
##### 4.2.1.3a.5 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure in the UL per DRB per cell
Protocol Layer: MAC
Table 4.2.1.3a.5-1: Definition for Max number of Active UEs in the UL per DRB per cell
| Definition | Maximum number of Active UEs in the UL per DRB per cell. The DRBs are mapped with the same 5QI. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs. Detailed Definition: $M\left( T,drbid,p \right)=\underset{T}{max}(\left( N\left( i,drbid \right) \right))$, where explanations can be found in the table 4.2.1.3a.5-2 below. |
| --- | --- |
...
##### 4.2.1.3a.6 Mean number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per cell
Protocol Layer: MAC, RLC
Table 4.2.1.3a.6-1: Definition for Mean number of Active UEs per cell
| Definition | Mean number of Active UEs per cell. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,p)=\frac{\left⌊ \frac{\sum_{\forall i} N(i)}{I(T,p)}*10 \right⌋}{10},$![](images/38.314_L2_Measurements_j00/fig-0011.png)where explanations can be found in the table 4.2.1.3a.6-2 below. |
| --- | --- |
...
##### 4.2.1.3a.7 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per cell
Protocol Layer: MAC, RLC
Table 4.2.1.3a.7-1: Definition for Max number of Active UEs per cell
| Definition | Maximum number of Active UEs per cell. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,p)=\underset{T}{max}(\left( N\left( i \right) \right))$![](images/38.314_L2_Measurements_j00/fig-0012.png), where explanations can be found in the table 4.2.1.3a.7-2 below. |
| --- | --- |
...
##### 4.2.1.3a.8 Mean number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per DRB per cell
Protocol Layer: MAC, RLC
Table 4.2.1.3a.8-1: Definition for Mean number of Active UEs per DRB per cell
| Definition | Mean number of Active UEs per DRB per cell. The DRBs are mapped with the same 5QI for NR SA. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,drbid,p)=\frac{\left⌊ \frac{\sum_{\forall i} N(i,drbid)}{I(T,p)}*10 \right⌋}{10}$, ![](images/38.314_L2_Measurements_j00/fig-0013.png)where explanations can be found in the table 4.2.1.3a.8-2 below. |
| --- | --- |
...
##### 4.2.1.3a.9 Max number of Active UEs in RRC_INACTIVE with ongoing SDT procedure per DRB per cell
Protocol Layer: MAC, RLC
Table 4.2.1.3a.9-1: Definition for Max number of Active UEs per DRB per cell
| Definition | Maximum number of Active UEs per DRB per cell. The DRBs are mapped with the same 5QI for NR SA. This measurement refers to UEs for which there is data available for transmission for the UL for DRBs, or there is data available for transmission for the DL for DRBs, or both. Detailed Definition: $M(T,drbid,p)=\underset{T}{max}(\left( N\left( i,drbid \right) \right))$![](images/38.314_L2_Measurements_j00/fig-0014.png), where explanations can be found in the table 4.2.1.3a.9-2 below. |
| --- | --- |
...
| Definition | Mean number of stored inactive UE contexts. Detailed Definition: $M(T,p)=\left⌊ \frac{\sum_{\forall i} N(i)}{I(T,p)} \right⌋$![](images/38.314_L2_Measurements_j00/fig-0015.png), where explanations can be found in the table 4.2.1.4.2-2 below. |
| --- | --- |
...
| Definition | Maximum number of stored inactive UE contexts. Detailed Definition: $M\left( T,p \right)= \underset{T}{max}(\left( N\left( i \right) \right))$![](images/38.314_L2_Measurements_j00/fig-0016.png), where explanations can be found in the table 4.2.1.4.3-2 below. |
| --- | --- |
...
| Definition | Uu Packet Loss Rate in the DL per DRB per UE. One packet corresponds to one RLC SDU. The measurement is done separately per DRB. Detailed Definition: $M(T,drbid)=\left⌊ \frac{Dloss(T,drbid)*1000000}{N(T,drbid)+Dloss(T,drbid)} \right⌋$, where explanations can be found in the table 4.2.1.5.1-2 below. |
| --- | --- |
...
| Definition | Uu Packet Loss Rate with delay threshold in the DL per DRB per UE: One packet corresponds to one RLC SDU. The measurement is done separately per DRB. Detailed definition: $$M_dt(T,drbid)=\left⌊ \frac{[Dloss\left( T,drbid \right)+Dexd\left( T, drbid \right)]*1000000}{N_dt\left( T,drbid \right)+Dloss\left( T,drbid \right)+Dexd\left( T, drbid \right)} \right⌋$$ Where explanations can be found in the table 4.2.1.5.2-2 below. |
| --- | --- |
...
| $$M_dt(T,drbid)$$ | Packet Loss Rate with delay threshold in the DL per DRB per UE. Unit: number of lost packets per transmitted packets per DRB * 106, Integer. Lost packets here means the packets that delayed more than delay threshold or not successfully transmitted. |
| --- | --- |
| $$Dloss(T,drbid)$$ | Number of DL packets, of a data radio bearer with DRB Identity = $drbid$, for which at least a part has been transmitted over the air but not positively acknowledged, and it was decided during time period $T$ that no more transmission attempts will be done. If transmission of a packet might continue in another cell, it shall not be included in this count. |
...
| $$N_dt(T,drbid)$$ | Number of DL packets, of a data radio bearer with DRB Identity = $drbid$, which has been transmitted over the air and positively acknowledged and delayed no more than the corresponding delay threshold during time period $T$. The delay threshold is as defined in Note 2. |
| $$T$$ | Time Period during which the measurement is performed, Unit: minutes. |
| $$drbid$$ | The identity of the measured DRB. |
...
| Definition | PDSCH PRB Usage for MIMO in the DL per cell is calculated in the time-frequency domain. Detailed Definition: $M\left( T \right)=\left⌊ \frac{\sum_{\forall i} \sum_{\forall j} {M1_{ij}(T)*L_{ij}(T)}}{\sum_{\forall j} {P_{j}(T)}*Alpha}*100 \right⌋,$where explanations can be found in the table 4.2.1.7.1-2 below. |
| --- | --- |
...
| Definition | PUSCH PRB Usage for MIMO in the UL per cell is calculated in the time-frequency domain. Detailed Definition: $M\left( T \right)=\left⌊ \frac{\sum_{\forall i} \sum_{\forall j} {M1_{ij}(T)*L_{ij}(T)}}{\sum_{\forall j} {P_{j}(T)}*Alpha}*100 \right⌋,$where explanations can be found in the table 4.2.1.7.2-2 below. |
| --- | --- |
...
| Definition | PDSCH PRB Usage based on statistical MIMO layer in the DL per cell is calculated in the time-frequency domain. Detailed Definition: $$M\left( T1 \right)=\left⌊ \frac{\sum_{\forall i} \sum_{\forall j} {M1_{ij}(T1)*L_{ij}(T1)}}{\sum_{\forall j} {P_{j}(T1)}*\beta}*100 \right⌋$$ $$\beta =\underset{T2}{max}(LaveDL(T))$$ explanations can be found in the table 4.2.1.7.3-2 below. |
| --- | --- |
...
| Definition | PDSCH PRB Usage for MIMO in the DL per cell is calculated in the time-frequency and spatial domain. Detailed Definition: $M\left( T \right)=\left\{ \begin{aligned}\left⌊ \frac{\sum_{\forall i} \sum_{\forall j} \left\{ M1_{ij}\left( T \right)*L_{ij}\left( T \right) \right\}}{\sum_{\forall j} \left\{ P_{j}\left( T \right) \right\}*LM\left( T \right)}*100 \right⌋ and LM\left( T \right)=\frac{\sum_{j} Lmax_{j}\left( T \right)}{K\left( T \right)};if K\left( T \right)\neq 0 \\ 0; Otherwise\end{aligned} \right.,$where $$K\left( T \right)=\sum_{j,Lmax_{j\left( T \right)}\neq 0} 1$$ explanations can be found in the table 4.2.1.7.5-2 below. |
| --- | --- |
...
| Definition | Total DL delay for DL PDCP duplicated packets per Split-DRB per UE. This measurement is applicable for EN-DC and SA. This measurement provides the total DL delay for a Split-DRB when PDCP duplication is enabled for the entire measurement period. Detailed Definition: $M_DL(T,drbid)=\min {(M_DL_{MCG}(T,drbid) , M_DL_{SCG}(T,drbid))}$, where explanations can be found in the table 4.2.1.9.1-2 below. |
| --- | --- |


Table 4.2.1.9.1-2: Parameter description for total DL delay for DL PDCP duplicated packets per Split-DRB per UE
| $$M_DL(T,drbid)$$ | Total DL Delay per Split-DRB per UE during the time period $T$. |
| --- | --- |
| $$M_DL_{MCG}(T,drbid)$$ | Total DL Delay on the MCG per split DRB per UE during the time period $T$. |
| $$M_DL_{SCG}(T,drbid)$$ | Total DL Delay on the SCG per split DRB per UE during the time period $T$. |
| $$\min {(M_DL_{MCG}(T,drbid) , M_DL_{SCG}(T,drbid))}$$ | Minimum of the total DL delays between MCG and SCG legs per Split-DRB per UE during the time period $T$. |
...
| Definition | Total UL delay for UL PDCP duplicated packets per Split-DRB per UE. This measurement is applicable for EN-DC and SA. This measurement provides the total UL delay for a Split-DRB when PDCP duplication is enabled for the entire measurement period. Detailed Definition: $M_UL(T,drbid)=\min {(M_UL_{MCG}(T,drbid) , M_UL_{SCG}(T,drbid))}$, where explanations can be found in the table 4.2.1.9.2-2 below. |
| --- | --- |


Table 4.2.1.9.2-2: Parameter description for total UL delay for UL PDCP duplicated packets per Split-DRB per UE
| $$M_UL(T,drbid)$$ | Total UL Delay per Split-DRB per UE during the time period $T$. |
| --- | --- |
| $$M_UL_{MCG}(T,drbid)$$ | Total UL Delay on the MCG per split DRB per UE during the time period $T$. |
| $$M_UL_{SCG}(T,drbid)$$ | Total UL Delay on the SCG per split DRB per UE during the time period $T$. |
| $$\min {(M_UL_{MCG}(T,drbid) , M_UL_{SCG}(T,drbid))}$$ | Minimum of the total UL delays between MCG and SCG legs per Split-DRB per UE during the time period $T$. |
...
| Definition | Total DL delay for DL PDCP non-duplicated packets per Split-DRB per UE. This measurement is applicable for EN-DC and SA. This measurement provides the total DL delay for a Split-DRB when PDCP duplication is not enabled for the entire measurement period. Detailed Definition: $$M_DL(T,drbid)=\frac{I_DL_{MCG}(T,drbid)\times M_DL_{MCG}(T,drbid)+I_DL_{SCG}(T,drbid)\times M_DL_{SCG}(T,drbid)}{I_DL(T,drbid)}$$ $I_DL\left( T,drbid \right)=I_DL_{MCG}(T,drbid)+I_DL_{SCG}(T,drbid)$, where explanations can be found in the table 4.2.1.9.3-2 below. |
| --- | --- |
...
| $$M_DL_{MCG}(T,drbid)$$ | Total DL Delay on the MCG per split DRB per UE during the time period $T$. |
| $$M_DL_{SCG}(T,drbid)$$ | Total DL Delay on the SCG per split DRB per UE during the time period $T$. |
| $$I_DL\left( T,drbid \right)$$ | Total number of DL PDCP non-duplicated packets sent per Split-DRB per UE during the time period $T$. |
| $$I_DL_{MCG}(T,drbid)$$ | Total number of DL PDCP non-duplicated packets sent on the MCG leg per Split-DRB per UE during the time period $T$, as defined in Table 4.2.1.8.1-1. |
| $$I_DL_{SCG}(T,drbid)$$ | Total number of DL PDCP non-duplicated packets sent on the SCG leg per Split-DRB per UE during the time period $T$, as defined in Table 4.2.1.8.1-1. |
...
| Definition | Total UL delay for UL PDCP non-duplicated packets per Split-DRB per UE. This measurement is applicable for EN-DC and SA. This measurement provides the total UL delay for a Split-DRB when PDCP duplication is not enabled for the entire measurement period. Detailed Definition: $$M_UL(T,drbid)=\frac{I_UL_{MCG}(T,drbid)\times M_UL_{MCG}(T,drbid)+I_UL_{SCG}(T,drbid)\times M_UL_{SCG}(T,drbid)}{I_UL(T,drbid)}$$ $I_UL\left( T,drbid \right)=I_UL_{MCG}(T,drbid)+I_UL_{SCG}(T,drbid)$, where explanations can be found in the table 4.2.1.9.4-2 below. |
| --- | --- |
...
| $$M_UL_{MCG}(T,drbid)$$ | Total UL Delay on the MCG per split DRB per UE during the time period $T$. |
| $$M_UL_{SCG}(T,drbid)$$ | Total UL Delay on the SCG per split DRB per UE during the time period $T$. |
| $$I_UL\left( T,drbid \right)$$ | Total number of UL PDCP non-duplicated packets sent per Split-DRB per UE during the time period $T$. |
| $$I_UL_{MCG}(T,drbid)$$ | Total number of UL PDCP non-duplicated packets sent on the MCG leg per Split-DRB per UE during the time period $T$, as defined in Table 4.2.1.8.1-1. |
| $$I_UL_{SCG}(T,drbid)$$ | Total number of UL PDCP non-duplicated packets sent on the SCG leg per Split-DRB per UE during the time period $T$, as defined in Table 4.2.1.8.1-1. |
...
| Definition | PDCCH CCE Usage per cell is calculated in the time-frequency-space domain. Detailed Definition: $M\left( T \right)=\left⌊ \frac{\sum_{\forall i} \sum_{\forall j} {M1_{ij}(T)*L_{ij}(T)}}{\sum_{\forall j} {P_{j}(T)}*Alpha}*100 \right⌋$, where explanations can be found in the table 4.2.1.10.1-2 below. |
| --- | --- |
...
##### 4.2.1.10.2 PDCCH CCE Usage based on statistical multi-UE spatial multiplexing per cell
...
| Definition | PDCCH CCE Usage based on statistical multi-UE spatial multiplexing per cell is calculated in the time-frequency-space domain. Detailed Definition: $$M\left( T1 \right)=\left⌊ \frac{\sum_{\forall i} \sum_{\forall j} {M1_{ij}(T1)}}{\sum_{\forall j} {P_{j}(T1)}*\beta}*100 \right⌋$$ $\beta =\underset{T2}{max}(Lave(T))$, where explanations can be found in the table 4.2.1.10.2-2 below. |
| --- | --- |


Table 4.2.1.10.2-2: Parameter description for PDCCH CCE Usage based on statistical multi-UE spatial multiplexing per cell
| $$M(T1)$$ | Total PDCCH CCE usage per cell which is percentage of CCEs, averaged during time period $T1$ with integer value. |
| --- | --- |
| $$M1_{ij}(T1)$$ | A count of PDCCH CCEs used for control information transmission for UE $i$ per cell at sampling occasion $j$. Counting unit for CCE is 6 Resource Block x 1 symbol. (1 Resource Block = 12 sub-carrier) |
| $$i$$ | A UE $i$ that is scheduled during time period 𝑇1. |
| $$j$$ | Sampling occasion during time period 𝑇1. A sampling occasion is 1 symbol. |
| $$P_{j}(T1)$$ | Total number of PDCCH CCEs available for sampling occasion $j$ per cell. |
| $$T1$$ | Time period during which the measurement is performed to calculate $M\left( T1 \right)$, e.g., 15min, 1 hour, etc. |
| $$\beta$$ | A variable factor for spatial multiplexing assigned with the maximum $Lave(T)$ during time period 𝑇2 with float value 1.00-100.00. |
| $$Lave(T)$$ | Average value of multi-UE spatial multiplexing per CCE on the DL during time period $T$ with float value 1.00-100.00. The concrete calculation procedure can be found in the table 4.2.1.10.2-3 below. |
| $$T$$ | Time period during which the measurement is performed to calculate$Lave(T)$. |
| $$T2$$ | Time period during which the measurement is performed to calculate β, e.g., 1 week, etc. |
...
| Definition | Average value of multi-UE spatial multiplexing per CCE per cell is calculated in the time-frequency-space domain. Detailed Definition: $Lave(T)=\frac{\sum_{\forall j} \sum_{\forall k} \left\{ M1_{kj}\left( T \right)*L_{kj}\left( T \right) \right\}}{\sum_{\forall j} \sum_{\forall k} \left\{ M1_{kj}\left( T \right) \right\}}$, where explanations can be found in the table 4.2.1.10.2-4 below. |
| --- | --- |


Table 4.2.1.10.2-4: Parameter description for average value of multi-UE spatial multiplexing per CCE
| $$Lave(T)$$ | The average value of multi-UE spatial multiplexing per CCE per cell with float value range: 1.00-100.00. |
| --- | --- |
| $$T$$ | Time period during which the measurement is performed to calculate$Lave(T)$, e.g., 15min, 1 hour, etc. |
| $$j$$ | Sampling occasion during time period $T$. A sampling occasion is 1 symbol. |
| $$k$$ | The number of kinds of spatial multiplexing. |
| $$L_{kj}(T)$$ | The number of spatial multiplexing for PDCCH CCEs at sampling occasion $j$. |
| $$M1_{kj}(T)$$ | The number of PDCCH CCEs used for control information transmission corresponding to $L_{kj}(T)$ at sampling occasion $j$. Counting unit for CCE is 6 Resource Block x 1 symbol. (1 Resource Block = 12 sub-carrier) |
