3GPP TS 38.215 V19.3.0 (2026-06)
...
(Release 19)
![](images/38.215_PHY_Measurements_j30/fig-0001.emf) ![](images/38.215_PHY_Measurements_j30/fig-0002.png)
...
5.1.50 Coherent joint transmission calibration delay offset 27
5.1.51 Coherent joint transmission calibration frequency offset 28
5.1.52 Coherent joint transmission calibration phase offset 28
5.1.53 Low power reference signal received power (LP-RSRP) 28
5.1.54 Low power reference signal received quality (LP-RSRQ) 29
5.1.55 Squared Generalized Cosine Similarity (SGCS) 29
5.2 NG-RAN measurement abilities 29
5.2.1 SSS transmit power 29
5.2.2 UL Relative Time of Arrival (TUL-RTOA) 30
5.2.3 gNB Rx – Tx time difference 30
5.2.4 UL Angle of Arrival (UL AoA) 31
5.2.5 UL SRS reference signal received power (UL SRS-RSRP) 31
5.2.6 UL SRS reference signal received path power (UL SRS-RSRPP) 32
5.2.7 Timing advance (TADV) 32
5.2.8 UL reference signal carrier phase (UL RSCP) 33
5.2.9 UL SRS time domain channel timing (UL SRS-TDCT) 33
5.2.10 UL SRS time domain channel power (UL SRS-TDCP) 34
...
| --- | --- |
| Applicable for | If CSI-RSRP is used for L1-RSRP, RRC_CONNECTED intra-frequency, RRC_CONNECTED inter-frequency. If CSI-RSRP is used for predicted RSRP, RRC_CONNECTED intra-frequency. Otherwise, RRC_CONNECTED intra-frequency, RRC_CONNECTED inter-frequency |
...
| Definition | The SL relative time of arrival (TSL-RTOA) is defined as the beginning time of SL subframe # i containing SL PRS received from a UE, relative to the relative time of arrival (RTOA) reference time. The SL RTOA reference time is defined as $T_{0}+t_{SL-PRS}$, where - $T_{0}$ is the nominal beginning time of SFN 0 or DFN 0, provided by SFN and DFN initialization time, respectively. - $t_{SL-PRS}=(10n_{f}+n_{sf})\times 10^{-3}$, where $n_{f}$ and $n_{sf}$are the SFN or DFN and the subframe number of the SL PRS, respectively. For frequency range 1, the reference point for TSL-RTOA measurement shall be the Rx antenna connector of the UE. For frequency range 2, the reference point for TSL-RTOA measurement shall be the Rx antenna of the UE. |
| --- | --- |
| Applicable for | Sidelink |
...
| Definition | Coherent joint transmission calibration delay offset is defined as the relative receive time difference, $D_{n,offset}$, between reference CSI-RS resource set, nref, and selected CSI-RS resource set and out-of-interval indicator, $d_{n}$. Out-of-interval indicator, $d_{n}$, indicates corresponding relative receive time difference plus delay spread is not larger than the duration of the cyclic prefix for symbols other than 0 and $7\cdot 2^{\mu }$ and set to ‘1’, otherwise set to ‘0’, Determination of reference CSI-RS resource set, nref, and determination of delay offset and out-of-interval indicator, labelled as ‘CJTC-Dd’, are describe in Clause 5.2.1.4.2 and 5.2.1.4.6 of [6, TS38.214], respectively. [For frequency range 1, the reference point for the coherent joint transmission calibration delay offset shall be the antenna connector of the UE.] |
| --- | --- |
| Applicable for | RRC_CONNECTED |
...
| Definition | Coherent joint transmission calibration phase offset is defined as the relative phase offset, $\Phi _{n}$ or $\Phi _{n,s },$between reference CSI-RS resource, nref, and selected CSI-RS resource over the entire configured reporting band or over the indicated subband, $s$, respectively. Determination of reference CSI-RS resource, nref, and determination of phase offset, labelled as ‘CJTC-P’, are describe in Clause 5.2.1.4.2 and 5.2.1.4.8 of [6, TS38.214], respectively. For frequency range 1, coherent joint transmission calibration phase offset shall be measured on the receiver branch(es) associated with the configured port for an SRS resource in the latest SRS transmission occasion before the occasions of the corresponding CSI-RS resources. [For frequency range 1, the reference point for the coherent joint transmission calibration phase offset shall be the antenna connector of the UE.] |
| --- | --- |
| Applicable for | RRC_CONNECTED |
...
| Definition | The UL Relative Time of Arrival (TUL-RTOA) is the beginning of subframe i containing SRS received in Reception Point (RP) [18] j, relative to the RTOA Reference Time [16]. The UL RTOA reference time is defined as $T_{0}+t_{SRS}$, where - $T_{0}$ is the nominal beginning time of SFN 0 provided by SFN Initialization Time [15, TS 38.455] - $t_{SRS}=\left( 10n_{f}+n_{sf} \right)\times 10^{-3}$, where $n_{f}$ and $n_{sf}$ are the system frame number and the subframe number of the SRS, respectively. Multiple SRS resources can be used to determine the beginning of one subframe containing SRS received at a RP. The reference point for TUL-RTOA shall be: - for type 1-C base station TS 38.104 [9]: the Rx antenna connector, - for type 1-O or 2-O base station TS 38.104 [9]: the Rx antenna (i.e. the centre location of the radiating region of the Rx antenna), - for type 1-H base station TS 38.104 [9]: the Rx Transceiver Array Boundary connector. |
| --- | --- |
...
| Definition | UL SRS time domain channel timing (UL SRS-TDCT) is defined as $\hat{N_{t}}$ values of timing information of the estimated channel response in time domain, $T_{n}$, corresponding to the resource elements carrying sounding reference signals (SRS), the $\hat{N_{t}}$ values are derived from $N_{t}$ consecutive time instances with timing granularity $2^{k}\cdot T_{c}$ that start from the first detected path rounded down with timing granularity $2^{k}\cdot T_{c}$, where $N_{t}$ and $\hat{N_{t}}$ are given by higher layers and $T_{c}$ is given in Clause 4.1 of [3, TS38.211], and k is the reporting granularity factor given by higher layer parameter timingReportingGranularityFactor. The $\hat{N_{t}}$ values of timing information are selected such that they correspond to the time instances with the highest power values among the $N_{t}$ consecutive time instances where a power value for a time instance is determined as the power of linear average of the estimated channel response from resource elements carrying sounding reference signals (SRS) at time instance $T_{n}$. For frequency range 1 and 2, if receiver diversity is in use by the gNB for power of linear average of the estimated channel response measurement: - The measured received signal power, $P_{1}$, value for the selected first time instance, $T_{1}$, shall not be lower than the corresponding received signal power for the selected time instance, $T_{1}$, of any of the individual receiver branches and the received signal power, $P_{n}$, shall be with the same receiver branch(es) as applied for the selected first time instance, $T_{1}$. The timing information are reported relative to reference time, $T_{ref}$. The reference time, $T_{ref}$, is equal to the UL RTOA reference time $T_{0}+t_{SRS}$, where - $T_{0}$ is the nominal beginning time of SFN 0 provided by SFN Initialization Time [15, TS 38.455] - $t_{SRS}=\left( 10n_{f}+n_{sf} \right)\times 10^{-3}$, where $n_{f}$ and $n_{sf}$ are the system frame number and the subframe number of the SRS, respectively. The reference point for UL SRS-TDCT shall be: - for type 1-C base station TS 38.104 [9]: the Rx antenna connector, - for type 1-O or 2-O base station TS 38.104 [9]: the Rx antenna (i.e. the centre location of the radiating region of the Rx antenna), - for type 1-H base station TS 38.104 [9]: the Rx Transceiver Array Boundary connector. |
| --- | --- |


### 5.2.10 UL SRS time domain channel power (UL SRS-TDCP)

| Definition | UL SRS time domain channel power (UL SRS-TDCP) is defined as a number of $\hat{N_{t}}$ values of power of the estimated channel response in time domain, $P_{n}$, where, for a given $n$, $P_{n}$ is defined as the power of the linear average of the estimated channel response at time instance $T_{n}$, for n = 1, …, $\hat{N_{t}}$, where time instance $T_{n}$is defined in Clause 5.2.9, from resource elements carrying sounding reference signals (SRS).. UL SRS-TDCP measurements may not be reported without reporting of UL SRS-TDCT measurements. The reference point for UL SRS-TDCP shall be: - for type 1-C base station TS 38.104 [9]: the Rx antenna connector, - for type 1-O or 2-O base station TS 38.104 [9]: the Rx antenna (i.e. the centre location of the radiating region of the Rx antenna), - for type 1-H base station TS 38.104 [9]: the Rx Transceiver Array Boundary connector. For frequency range 1 and 2, if receiver diversity is in use by the gNB for UL SRS-TDCP measurements: - The reported received signal power, $P_{1}$, value for the selected first time instance, $T_{1}$, shall not be lower than the corresponding received signal power for the selected time instance, $T_{1}$, of any of the individual receiver branches and the received signal power, $P_{n}$, shall be provided for the same receiver branch(es) as applied for the selected first time instance, $T_{1}$. |
| --- | --- |
...
| 2026-06 | RAN#112 | RP-261348 | 0079 | - | A | CR on applicable scenarios of SS-RSRP for LTM | 19.3.0 |
| 2026-06 | RAN#112 | RP-261344 | 0081 | - | F | Correction on SS-RSRP/CSI-RSRP for LTM | 19.3.0 |
